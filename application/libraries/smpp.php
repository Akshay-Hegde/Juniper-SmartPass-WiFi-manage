...
class Smpp {
  ...
  public function __construct ($param = array())
  {
  		if (count($param) > 0)
  		{
		    $this->server = $param['smpp_server'];
		    $this->port = $param['smpp_port'];
		    $this->user = $param['smpp_user'];
		    $this->pass = $param['smpp_pass'];
		    $this->src_number = $param['smpp_src_number'];
		}
		else
		{
			die("i can't to start to use smpp library without parameters");
		}
  }
